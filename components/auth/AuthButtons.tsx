...
const AuthButtons: React.FC<AuthButtonsProps> = ({ isMobile = false }) => {
  ...
  if (status === 'unauthenticated') {
    return (
      <button onClick={() => signIn('google', { callbackUrl: '/dashboard' })} className="px-4 py-2 bg-gradient-to-tr from-pink-500 to-yellow-400 text-white rounded-xl shadow-lg hover:scale-105 transition-all">Sign In</button>
    );
  }
